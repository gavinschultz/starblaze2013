...
void AlienSprite::render(SDL_Renderer* sdl_renderer, const Camera &camera)
{
	...
	SDL_Rect dstrect = {
		render::getScreenXForEntityByCameraAndDistance(_alien->alpha_pos.x*_scaling, _alien_texture_rect.w*_scaling, renderer->width, camera, 1.0),
...
void RadarSprite::render(SDL_Renderer* sdl_renderer, const Camera& camera)
{
	...
	if (alien)
	{
		SDL_Rect point_rect = transformToRadarView(alien->alpha_pos.x, alien->alpha_pos.y, alien->bounding_box.w, alien->bounding_box.h);
	...
	if (station)
	{
		SDL_Rect point_rect = transformToRadarView(station->pos.x, station->pos.y, station->bounding_box.w, station->bounding_box.h);
...
SDL_Rect RadarSprite::transformToRadarView(double entity_x, double entity_y, int entity_width, int entity_height)
{
	// X
...
void StationSprite::render(SDL_Renderer* sdl_renderer, const Camera& camera)
{
	...
	int station_x = render::getScreenXForEntityByCameraAndDistance(station->pos.x*_scaling, _station_texture_rect.w*_scaling, renderer->width, camera, 1.0);
