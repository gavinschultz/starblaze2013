...
void SpriteLoader::load(SDL_Renderer* sdl_renderer, const std::string& spritesheet_json_path)
{
	...
	if (pi->texture)
	{
		SDL_DestroyTexture(pi->texture);
		pi->texture = nullptr;
	}

	pi->texture = renderutil::loadTextureFromFile(sdl_renderer, "resources\\" + spritesheet_image_filename, &pi->texture_rect);
...
const MappedTexture& SpriteLoader::getSprite(const std::string& name) const
{
	...
	return pi->sprites_by_name[name];
}