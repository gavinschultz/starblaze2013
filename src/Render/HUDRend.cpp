...
HUDRend::HUDRend(Renderer* renderer) : Sprite(renderer->scaling), pimpl{ new impl{} }
{
	...
	pimpl->gauge_color = renderer->coco_palette.getColor(CoCoPaletteEnum::blue);
}
...
void HUDRend::render(SDL_Renderer* sdl_renderer, const TextRenderer& text_renderer, const Ship& ship, int score, int player_ships) const
{
	...
	text_renderer.RenderChar(sdl_renderer, 'F', { 2 * (int32_t)_scaling, 184 * (int32_t)_scaling }, true);
	text_renderer.RenderChar(sdl_renderer, 'T', { 18 * (int32_t)_scaling, 184 * (int32_t)_scaling }, true);
	text_renderer.RenderChar(sdl_renderer, 'S', { 34 * (int32_t)_scaling, 184 * (int32_t)_scaling }, true);
	text_renderer.RenderChar(sdl_renderer, 'R', { 50 * (int32_t)_scaling, 184 * (int32_t)_scaling }, true);
