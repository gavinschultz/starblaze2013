...
void AlienRender::render(SDL_Renderer* sdl_renderer, const Camera& camera, const TemporalState2DComponent& state, const CollisionComponent* collide) const
{
	...
	if (collide->is_collided)
		SDL_SetTextureColorMod(pi->alien_texture.texture, 0, 0, 0);
	else
		SDL_SetTextureColorMod(pi->alien_texture.texture, 255, 255, 255);
